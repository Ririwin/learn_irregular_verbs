...
let streakGoal = 25;
let selectedGroups = [];
let isCheckingAnswer = false; // Flag pour éviter les actions pendant la vérification
...
function construireCasesGroupesDynamiques() {
  ...
  for (let i = 0; i < groups.length; i++) {
    ...
    cb.type = 'checkbox';
    cb.id = 'group-' + i;
    cb.checked = false; // MODIFICATION: Tous les checkboxes sont maintenant décochés par défaut
...
function checkAnswer() {
  ...
  isCheckingAnswer = true; // MODIFICATION: Flag indiquant que la réponse a été vérifiée
}
...
// MODIFICATION: Écouter la touche Entrée pour passer à la question suivante
document.addEventListener('keydown', function(event) {
  if (event.key === 'Enter' && isCheckingAnswer && nextButton.style.display === 'block') {
